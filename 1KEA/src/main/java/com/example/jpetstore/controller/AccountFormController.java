	...
	public AccountForm formBackingObject(HttpServletRequest request) 
		...
		if (userSession != null) {	// edit an existing account
			return new AccountForm(
				oneKea.getAccount(userSession.getAccount().getUsername()));
		}
	...
	public String onSubmit(
		...
		try {
			if (accountForm.isNewAccount()) {
				oneKea.insertAccount(accountForm.getAccount());
			}
			else {
				oneKea.updateAccount(accountForm.getAccount());
			}
		}
		...
		UserSession userSession = new UserSession(
			oneKea.getAccount(accountForm.getAccount().getUsername()));
		PagedListHolder<Product> myList = new PagedListHolder<Product>(
			oneKea.getProductListByCategory(accountForm.getAccount().getFavouriteCategoryId()));
